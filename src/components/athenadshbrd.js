...
function AthenaDashboard() {
  ...
  const getFilterOptions = (data, key) => {
    ...
    const uniqueValues = new Set(data.data.map((row) => row[key] || ''));
  ...
  const typeDistributionData = React.useMemo(() => {
    ...
    fullRows.forEach((row) => {
      const t = row.type || row.restorepoints || 'Unknown';
            ...
            <Box display="flex" flexWrap="wrap" gap={2}>
              <Paper style={{ flex: 1, minWidth: 300, padding: 16 }} elevation={3}>
                <Typography variant="subtitle1" gutterBottom>
                  {selectedTable === 'csv_data' ? 'Storage Type Distribution' : 'Restore Points Distribution'}
                </Typography>
                <Box height={250}>
              ...
              <Paper style={{ flex: 1, minWidth: 300, padding: 16 }} elevation={3}>
                <Typography variant="subtitle1" gutterBottom>
                  {selectedTable === 'csv_data' ? 'Used Data % (Line Chart)' : 'Col3 Value (Line Chart)'}
                </Typography>
                <Box height={250}>
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart
                    ...
                    >
                      ...
                      <Line
                        ...
                        stroke="#8884d8"
                        strokeWidth={2}
                        name={selectedTable === 'csv_data' ? 'Used Data %' : 'Col3'}
                      />
                    </LineChart>
                  </ResponsiveContainer>
                </Box>
